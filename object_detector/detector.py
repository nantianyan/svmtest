...
pyramid_gaussian_scale_downscale = 1.25
pyramid_gaussian_scale_min_wdw_sz = (64, 128)
pyramid_gaussian_scale_decision_function = 0.6  #0.5
sliding_window_step_size = (10, 10)

non_max_suppression_overlapThresh = 0.3
...
def detector(filename):
    ...
    if im is None:
        print('im is None:' + filename )
        return
    im = imutils.resize(im, width = min(400, im.shape[1]))
    min_wdw_sz = pyramid_gaussian_scale_min_wdw_sz
    step_size = sliding_window_step_size
    downscale = pyramid_gaussian_scale_downscale
    ...
    for im_scaled in pyramid_gaussian(im, downscale = downscale):
        ...
        for (x, y, im_window) in sliding_window(im_scaled, min_wdw_sz, step_size):
            ...
            if pred == 1:
                ...
                if clf.decision_function(fd) > pyramid_gaussian_scale_decision_function:
    ...
    print("sc: ", sc)
    sc = np.array(sc)
    pick = non_max_suppression(rects, probs = sc, overlapThresh = non_max_suppression_overlapThresh)
